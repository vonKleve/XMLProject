...
BigInteger::BigInteger(std::string stringInteger)
{
...
BigInteger BigInteger::operator+(long long const &b) const
{
	BigInteger c = *this;
	c += b;
...
BigInteger BigInteger::operator*(long long const &b)
{
	BigInteger c = *this;
	c *= b;
...
BigInteger &BigInteger::operator*=(int const &b)
{
	std::vector<int>::iterator it = number.begin();
	long long sum = 0;
	while (it != number.end()) {
...
int BigInteger::operator%(int const & divisor)
{
	long int remains = 0;
...
int BigInteger::trailing_zeros() const
{
...
std::string to_string(BigInteger const &value)
{
